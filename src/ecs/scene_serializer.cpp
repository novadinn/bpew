    ...
    if (mesh.meshes.size() > 0) {
      ...
      for (int i = 0; i < mesh.meshes.size(); ++i) {
        ...
        if (MaterialManager::validMaterialIndex(
                mesh.meshes[i].active_material_index)) {
...
void SceneSerializer::serializeMaterial(YAML::Emitter &out,
                                        Material &material) {
  out << YAML::BeginMap;
      ...
      if (node.inputs.size() > 0) {
        ...
        for (auto &input : node.inputs) {
          ...
          if (input.link) {
            ...
            out << YAML::Key << "OutputNodeIndex" << YAML::Value
                << input.link->output_node_index;
            out << YAML::Key << "InputNodeIndex" << YAML::Value
                << input.link->input_node_index;
            out << YAML::Key << "OutputIndex" << YAML::Value
                << input.link->output_index;
            out << YAML::Key << "InputIndex" << YAML::Value
                << input.link->input_index;
        ...
        for (auto &output : node.outputs) {
          ...
          if (output.links.size() > 0) {
            ...
            for (auto &link : output.links) {
              ...
              out << YAML::Key << "OutputNodeIndex" << YAML::Value
                  << link.output_node_index;
              out << YAML::Key << "InputNodeIndex" << YAML::Value
                  << link.input_node_index;
              out << YAML::Key << "OutputIndex" << YAML::Value
                  << link.output_index;
              out << YAML::Key << "InputIndex" << YAML::Value
                  << link.input_index;
