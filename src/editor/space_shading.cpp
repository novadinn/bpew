...
void drawNodeMix(SpaceShadingData *ctx, Node& node) {
	...
	case NodePropertyType::FLOAT:
	    drawNodeInputFloatDrag(ctx, input, input.id.name.c_str(), FLT_MIN, FLT_MAX);
	    break;
	case NodePropertyType::VECTOR3:	    
	    drawNodeInputFloatDrag3(ctx, input, input.id.name.c_str(), FLT_MIN, FLT_MAX);
	    break;
...
void drawNodeInputFloatDrag(SpaceShadingData *ctx, NodeInput& input, const char* title, float min, float max) {
    if(!input.useUniform(ctx->activeMaterial()))
	return;
    ImGui::DragFloat(title, &input.value.float_value, 0.001f, min, max);
}

void drawNodeInputFloatDrag3(SpaceShadingData *ctx, NodeInput& input, const char* title, float min, float max) {
    if(!input.useUniform(ctx->activeMaterial()))
	return;
...
void drawNodeInputColor3Picker(SpaceShadingData *ctx, NodeInput& input, const char* title, ImGuiColorEditFlags flags) {    
    if(!input.useUniform(ctx->activeMaterial()))
	return;
    static ImGuiColorEditFlags color_flags = ImGuiColorEditFlags_PickerHueBar	    	   
	| ImGuiColorEditFlags_NoAlpha | flags;
...
void drawNodeInputColor3Edit(SpaceShadingData *ctx, NodeInput& input, const char* title, ImGuiColorEditFlags flags) {
    if(!input.useUniform(ctx->activeMaterial()))
	return;
    static ImGuiColorEditFlags color_flags = ImGuiColorEditFlags_PickerHueBar	    	   
...
void onDrawUISpaceShading(EditorContext *ctx) {
    SpaceShadingData *space_data = ctx->space_shading_data;
    Material* mat = MaterialManager::getMaterial(space_data->active_material_index);
	...
	if(ImGui::Button("Destroy Material")) {
	    if(MaterialManager::validMaterialIndex(space_data->active_material_index)) {
	...
	if(ImGui::CollapsingHeader("Materials")) {
		...
		if(ImGui::Selectable(mat.name.c_str(), space_data->active_material_index == i, ImGuiSelectableFlags_DontClosePopups)) {
