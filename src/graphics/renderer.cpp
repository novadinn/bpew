...
void Renderer::init() {
  ShaderBuilder::buildShaderFromCreateInfo(solid_shader,
                                           solid_shader_create_info);
  ShaderBuilder::buildShaderFromCreateInfo(fxaa_shader,
                                           fxaa_shader_create_info);
  ShaderBuilder::buildShaderFromCreateInfo(
      outline_selected_mesh_shader, outline_selected_mesh_shader_create_info);
  ...
  quad_mesh.attributes = {{sizeof(float), 2, GL_FALSE},
...
void Renderer::drawMeshMaterial(RendererContext *context) {
  ...
  if (!material || !material->shader_container ||
      !material->shader_container->compiled) {
  ...
  for (int i = 0; i < context->mesh->meshes.size(); ++i) {
    ...
    material->shader_container->shader.setMatrix4("projection",
                                                  context->projection);
    ...
    material->shader_container->shader.setVec3("dirLight.direction",
                                               context->direction);
    material->shader_container->shader.setVec3("dirLight.ambient",
                                               glm::vec3(0.05f, 0.05f, 0.05f));
    material->shader_container->shader.setVec3("dirLight.diffuse",
                                               glm::vec3(0.4f, 0.4f, 0.4f));
    material->shader_container->shader.setVec3("dirLight.specilar",
                                               glm::vec3(0.5f, 0.5f, 0.5f));
    material->shader_container->shader.setInt("currentEntityID",
                                              (int)context->current_entity_id);
  ...
  for (int i = 0; i < context->mesh->meshes.size(); ++i) {
    ...
    for (int j = 0; j < context->lights.size(); ++j) {
      ...
      case LightComponent::LightType::DIRECTIONAL: {
        std::string element =
            "dirLights[" + std::to_string(num_dir_lights++) + "]";

        std::string value = element + ".direction";
        material->shader_container->shader.setVec3(
            value.c_str(), GLMExtensions::modelToForward(
                               context->light_transforms[j].getModelMatrix()));
        value = element + ".ambient";
        material->shader_container->shader.setVec3(value.c_str(),
                                                   context->lights[j].ambient);
        value = element + ".diffuse";
        material->shader_container->shader.setVec3(value.c_str(),
                                                   context->lights[j].diffuse);
      ...
      case LightComponent::LightType::POINT: {
        std::string element =
            "pointLights[" + std::to_string(num_point_lights++) + "]";

        std::string value = element + ".position";
        material->shader_container->shader.setVec3(
            value.c_str(), context->light_transforms[j].position);
        value = element + ".ambient";
        material->shader_container->shader.setVec3(value.c_str(),
                                                   context->lights[j].ambient);
        value = element + ".diffuse";
      ...
      case LightComponent::LightType::SPOT: {
        std::string element =
            "spotLights[" + std::to_string(num_spot_lights++) + "]";
        ...
        material->shader_container->shader.setFloat(
            value.c_str(), context->lights[j].properties.spot_light.cut_off);
        value = element + ".outerCutOff";
        material->shader_container->shader.setFloat(
            value.c_str(),
            context->lights[j].properties.spot_light.outer_cut_off);
        value = element + ".ambient";
        material->shader_container->shader.setVec3(value.c_str(),
                                                   context->lights[j].ambient);
        value = element + ".diffuse";
...
void Renderer::drawMeshWireframe(RendererContext *context) {
  /* TODO: need to create our own shader, since mouse picking is not working
    correctly with this kind of wireframe */
...
void Renderer::drawMeshVertices(RendererContext *context) {
  /* TODO: move that to shader, since gl_PointSize is overritable from vertex
   * shader */
  ...
  for (int i = 0; i < context->mesh->meshes.size(); ++i) {
    ...
    mesh_vertices_shader.setInt("currentEntityID",
                                (int)context->current_entity_id);
...
void Renderer::applyMeshOutline(RendererContext *context) {
  ...
  outline_selected_mesh_shader.setInt("selectedEntityID",
                                      (int)context->selected_entity_id);
  outline_selected_mesh_shader.setVec3("selectionColor",
...
void Renderer::applyVertexOutline(RendererContext *context) {
  ...
  outline_selected_vertex_shader.setInt("selectedEntityID",
                                        (int)context->selected_entity_id);
  outline_selected_vertex_shader.setInt("selectedVertexID",
                                        (int)context->selected_vertex_id);
  outline_selected_vertex_shader.setVec3("selectionColor",
                                         context->outline_color);
...
void Renderer::setMaterialNodeUniform(Shader &shader, Node &node,
                                      NodeProperty &prop) {
