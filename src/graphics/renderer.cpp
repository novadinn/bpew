...
void Renderer::init() {
    ...
    ShaderBuilder::buildShaderFromCreateInfo(rendered_shader, rendered_shader_create_info);
...
void Renderer::drawMeshRendered(uint32 entity_id, MeshComponent& mesh, glm::mat4& view_mat,
				...
				const glm::mat4& model) {
    for(int i = 0; i < mesh.model.meshes.size(); ++i) {
	rendered_shader.bind();
	rendered_shader.setMatrix4("view", view_mat);
	rendered_shader.setMatrix4("projection", proj_mat);
	rendered_shader.setMatrix4("model", model);

	rendered_shader.setFloat("shininess", 0.2f);
	rendered_shader.setVec3("viewPos", view_pos);
	rendered_shader.setInt("entity", (int)entity_id);
	...
	for(int j = 0; j < lights.size(); ++j) {
		...
		rendered_shader.setVec3(value.c_str(), GLMExtensions::modelToForward(light_transforms[j].getModelMatrix()));
		value = element + ".ambient";
		rendered_shader.setVec3(value.c_str(), lights[j].ambient);
		value = element + ".diffuse";
		rendered_shader.setVec3(value.c_str(), lights[j].diffuse);
		value = element + ".specular";
		rendered_shader.setVec3(value.c_str(), lights[j].specular);
	    } break;
	    case LightComponent::LightType::POINT: {
		std::string element = "pointLights[" + std::to_string(num_point_lights++) + "]";
				
		std::string value = element + ".position";
		rendered_shader.setVec3(value.c_str(), light_transforms[j].position);
		value = element + ".ambient";
		rendered_shader.setVec3(value.c_str(), lights[j].ambient);
		value = element + ".diffuse";
		rendered_shader.setVec3(value.c_str(), lights[j].diffuse);
		...
		rendered_shader.setFloat(value.c_str(), lights[j].properties.point_light.constant);
		value = element + ".linear";
		rendered_shader.setFloat(value.c_str(), lights[j].properties.point_light.linear);
		value = element + ".quadratic";
		rendered_shader.setFloat(value.c_str(), lights[j].properties.point_light.quadratic);
	    } break;
		...
		rendered_shader.setFloat(value.c_str(), lights[j].properties.spot_light.cut_off);
		value = element + ".outerCutOff";
		rendered_shader.setFloat(value.c_str(), lights[j].properties.spot_light.outer_cut_off);
		value = element + ".ambient";
		rendered_shader.setVec3(value.c_str(), lights[j].ambient);
		value = element + ".diffuse";
		rendered_shader.setVec3(value.c_str(), lights[j].diffuse);
		...
		rendered_shader.setVec3(value.c_str(), lights[j].specular);
		value = element + ".constant";
		rendered_shader.setFloat(value.c_str(), lights[j].properties.spot_light.constant);
		value = element + ".linear";
		rendered_shader.setFloat(value.c_str(), lights[j].properties.spot_light.linear);
		value = element + ".quadratic";
		rendered_shader.setFloat(value.c_str(), lights[j].properties.spot_light.quadratic);
	    } break;
	    };
	}
