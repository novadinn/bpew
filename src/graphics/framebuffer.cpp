...
void Framebuffer::blitTo(Framebuffer other) {
  ...
  glBlitFramebuffer(0, 0, data.width, data.height, 0, 0, data.width,
                    data.height, GL_COLOR_BUFFER_BIT, GL_NEAREST);
}
...
bool Framebuffer::resize(uint width, uint height) {
  if (width == 0 || height == 0 || width > MAX_FRAMEBUFFER_SIZE ||
    ...
    LOG_ERROR("Can not resize framebuffer more that %d. Attempted width: %d, "
              "height: %d\n",
...
void Framebuffer::clearR32IColorAttachment(uint index, int value) {
  ...
  glClearTexImage(color_attachment_ids[index], 0, GL_RED_INTEGER, GL_INT,
                  &value);
}
...
void Framebuffer::clearRGBA8ColorAttachment(uint index, glm::vec4 value) {
  ...
  glClearTexImage(color_attachment_ids[index], 0, GL_RGBA, GL_FLOAT,
                  glm::value_ptr(value));
}
...
bool Framebuffer::invalidate() {
  ...
  if (color_attachment_formats.size()) {
    ...
    glCreateTextures(texture_target, color_attachment_ids.size(),
                     &color_attachment_ids[0]);
  ...
  if (color_attachment_ids.size() > 1) {
    if (color_attachment_ids.size() > 4) {
      LOG_ERROR(
          "Number of frambuffer color attachments exceeded maximum of %d\n", 4);
    }
...
void Framebuffer::attachColorTexture(GLenum internal_format, GLenum format,
                                     int index) {
  bool multisampled = data.samples > 1;
  GLenum texture_target =
      multisampled ? GL_TEXTURE_2D_MULTISAMPLE : GL_TEXTURE_2D;
  ...
  if (multisampled) {
    glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, data.samples, GL_RGBA8,
                            data.width, data.height, GL_TRUE);
  } else {
    glTexImage2D(GL_TEXTURE_2D, 0, internal_format, data.width, data.height, 0,
